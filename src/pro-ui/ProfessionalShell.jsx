...
// ── Tool Strip Items ──────────────────────────────────────────────────────────
const TOOLS = [
  { id: "select",    label: "Select (S)",     fn: "select",    svg: <svg viewBox="0 0 24 24" fill="currentColor"><path d="M4 2L4 18L8 14L11 21L13 20L10 13L15 13Z"/></svg> },
  { id: "move",      label: "Move (G)",       fn: "grab",      svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 4L12 20M4 12L20 12M12 4L10 7M12 4L14 7M12 20L10 17M12 20L14 17M4 12L7 10M4 12L7 14M20 12L17 10M20 12L17 14"/></svg> },
  { id: "rotate",    label: "Rotate (R)",     fn: "rotate",    svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 12A8 8 0 1 1 14 4.5"/><polyline points="14,2 14,5 17,5"/></svg> },
  { id: "scale",     label: "Scale (S)",      fn: "scale",     svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 12L20 4M20 4L16 4M20 4L20 8M12 12L4 20M4 20L8 20M4 20L4 16"/><circle cx="12" cy="12" r="2" fill="currentColor"/></svg> },
  { id: "div1", divider: true },
  { id: "extrude",   label: "Extrude (E)",    fn: "extrude",   svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="6" y="13" width="12" height="7" rx="1"/><path d="M12 13L12 4M9 7L12 4L15 7"/></svg> },
  { id: "loop_cut",  label: "Loop Cut",       fn: "loop_cut",  svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="4" y="6" width="16" height="12" rx="1"/><line x1="12" y1="6" x2="12" y2="18" strokeWidth="2"/></svg> },
  { id: "knife",     label: "Knife (K)",      fn: "knife",     svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M6 18L18 6M14 4L20 10L8 20L4 20L4 16Z"/></svg> },
  { id: "edge_slide",label: "Edge Slide",     fn: "edge_slide",svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="4" y1="12" x2="20" y2="12"/><circle cx="9" cy="12" r="2.5" fill="currentColor"/><polyline points="16,9 20,12 16,15"/></svg> },
  { id: "bevel",     label: "Bevel (Ctrl+B)", fn: "bevel",     svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 20L4 8L8 4L20 4"/><path d="M4 20L20 4" strokeDasharray="2,2"/></svg> },
  { id: "inset",     label: "Inset (I)",      fn: "inset",     svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="4" y="4" width="16" height="16" rx="1"/><rect x="8" y="8" width="8" height="8" rx="1"/></svg> },
  { id: "div2", divider: true },
  { id: "annotate",  label: "Annotate",       fn: "annotate",  svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M4 20Q8 12 12 14Q16 16 20 4"/></svg> },
  { id: "measure",   label: "Measure",        fn: "measure",   svg: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><line x1="4" y1="20" x2="20" y2="4"/><line x1="4" y1="20" x2="7" y2="17"/><line x1="20" y1="4" x2="17" y2="7"/></svg> },
];

// ── Workspace Tools Dock Data ─────────────────────────────────────────────────
const DOCK_TOOLS = [
  { label: "Cloth Sim", fn: "cloth_sim_start" },
  { label: "Fluid Sim", fn: "fluid_sim_start" },
  { label: "APIC GPU Fluid", fn: "apic_fluid_start" },
  { label: "Particles", fn: "particle_emit" },
  { label: "Fire", fn: "particle_fire" },
  { label: "Smoke", fn: "particle_smoke" },
  { label: "Subdivide", fn: "modifier_add" },
  { label: "Apply Mods", fn: "modifier_apply_all" },
  { label: "Smart UV", fn: "smart_uv" },
];
...
// ── WorkspaceToolsDock ────────────────────────────────────────────────────────
function WorkspaceToolsDock({ onAction }) {
  const [active, setActive] = useState(null);
  return (
    <div className="spx-tools-dock">
      {DOCK_TOOLS.map((t, i) => (
        <button
          key={t.fn}
          className={`spx-dock-btn${active === t.fn ? " spx-dock-btn--active" : ""}`}
          onClick={() => { setActive(t.fn); onAction?.(t.fn); }}
        >
          {t.label}
        </button>
      ))}
    ...
    <div className="spx-shell">
      ...
      <div className="spx-menubar">
        ...
        <div className="spx-menubar-right">
          ...
          <span className="spx-engine-badge">v2.0</span>
        </div>
      </div>
      ...
      <div className="spx-workspaces">
        {WORKSPACES.map((ws) => (
          <button
            key={ws}
            className={`spx-workspace-tab${ws === activeWorkspace ? " spx-workspace-tab--active" : ""}`}
      ...
      {/* Tools Dock */}
      <WorkspaceToolsDock onAction={onMenuAction} />
      ...
      <div className="spx-statusbar">
        <span className="spx-status-workspace">{(activeWorkspace || "MODELING").toUpperCase()}</span>
