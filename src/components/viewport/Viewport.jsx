...
import * as CurveSystem from "../../mesh/CurveSystem";
...
import { registerPlugin, unregisterPlugin, getPlugins, getAllPlugins } from "../../mesh/PluginSystem";
...
export default function Viewport(props) {
  ...
  const orbitRef    = useRef({ theta:0.6, phi:1.1, radius:12, target:new THREE.Vector3(0,0,0) });
  ...
  useEffect(() => {
    ...
    cam.position.set(7,5,12); cam.lookAt(0,0,0);
    camRef.current = cam;

    const grid = new THREE.GridHelper(30, 30, 0x00ffc8, 0x2a4a5a);
    grid.name = "_grid";
    grid.visible = true;
    scene.add(grid);
    // X axis line (red)
